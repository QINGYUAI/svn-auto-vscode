...
- 🎯 **Cursor 编辑器支持**：自动检测 Cursor 编辑器，优先使用 Cursor 内置 AI 功能
...
插件会自动检测当前编辑器是否为 Cursor。如果检测到 Cursor 编辑器，插件会**优先使用 Cursor 内置的 AI 功能**，而不是使用配置的 AI 服务。

**工作原理：**
- 插件会自动检测编辑器类型
- 如果检测到 Cursor，会尝试调用 Cursor 的 AI API
- 如果 Cursor AI 不可用，会自动回退到配置的 AI 服务
- 如果未检测到 Cursor，则使用配置的 AI 服务

**优势：**
- 无需配置 API 密钥即可使用 Cursor 的 AI 功能
- 自动优先使用 Cursor 内置 AI，提供更好的集成体验
- 如果 Cursor AI 不可用，自动回退到配置的 AI，确保功能可用
...
**Cursor 编辑器工作流程：**
```
修改代码 → 执行提交 → 选择文件 → 检测到Cursor → 使用Cursor AI → 生成提交信息 → 确认提交
                                      ↓ (如果Cursor AI不可用)
                                    回退到配置的AI → 生成提交信息 → 确认提交
```
