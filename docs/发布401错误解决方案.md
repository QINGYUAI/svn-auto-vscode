# 🔧 发布时 401 错误解决方案

## ❌ 错误信息

```
INFO  Publishing 'QINGYUAI.svn-git-auto-commit v0.2.0'...
ERROR  Failed request: (401)
```

## 🔍 问题分析

401 错误表示**认证失败**。虽然 `vsce login` 成功，但发布时仍然失败，可能的原因：

1. **Token 权限不足** - 登录只需要基本权限，但发布需要 `Marketplace: Manage` 权限
2. **Token 已过期** - Token 可能在使用过程中过期
3. **Publisher 权限问题** - Publisher 可能没有发布权限
4. **Token 缓存问题** - 旧的 Token 可能被缓存

## ✅ 解决方案

### 方案 1: 重新创建 Token（推荐）

1. **访问 Token 管理页面**：
   ```
   https://dev.azure.com/{组织名称}/_usersSettings/tokens
   ```
   如果组织名称是 `QINGYUAI`，访问：
   ```
   https://dev.azure.com/QINGYUAI/_usersSettings/tokens
   ```

2. **删除旧 Token**

3. **创建新 Token**，确保勾选：
   ```
   ☑ Marketplace (Manage)          # 必需：发布扩展
   ☑ User Profile (Read)            # 必需：验证用户
   ☑ Organization (Read)            # 推荐
   ```

4. **重新登录**：
   ```bash
   vsce logout QINGYUAI
   vsce login QINGYUAI
   # 粘贴新 Token
   ```

5. **验证权限**：
   ```bash
   vsce verify-pat QINGYUAI
   ```

6. **重新发布**：
   ```bash
   vsce publish
   ```

### 方案 2: 检查 Publisher 权限

1. **访问 Publisher 管理页面**：
   ```
   https://marketplace.visualstudio.com/manage/publishers/QINGYUAI
   ```

2. **检查 Publisher 状态**：
   - 确认 Publisher 已创建
   - 确认 Publisher 处于活动状态
   - 检查是否有任何限制

### 方案 3: 使用环境变量设置 Token

如果登录有问题，可以尝试使用环境变量：

```powershell
# Windows PowerShell
$env:VSCE_PAT="你的Token"
vsce publish
```

```bash
# Linux/Mac
export VSCE_PAT="你的Token"
vsce publish
```

### 方案 4: 清除缓存并重新登录

```bash
# 登出
vsce logout QINGYUAI

# 清除 vsce 缓存（如果存在）
# Windows: 删除 %USERPROFILE%\.vsce
# Linux/Mac: 删除 ~/.vsce

# 重新登录
vsce login QINGYUAI
```

## 🔍 诊断步骤

### 步骤 1: 验证 Token 权限

```bash
vsce verify-pat QINGYUAI
```

**期望输出**：
```
The Personal Access Token verification succeeded for the publisher 'QINGYUAI'.
```

如果失败，说明 Token 权限不足。

### 步骤 2: 检查 Publisher

访问：https://marketplace.visualstudio.com/manage/publishers

确认 `QINGYUAI` 存在且状态正常。

### 步骤 3: 尝试打包（不发布）

```bash
vsce package
```

如果打包成功，说明配置正确，问题在发布权限。

### 步骤 4: 检查版本号

确保版本号是新的，不能发布已存在的版本：

```bash
# 检查当前版本
cat package.json | grep version

# 如果版本已存在，更新版本号
```

## 📋 完整检查清单

发布前确认：

- [ ] Token 有 `Marketplace: Manage` 权限
- [ ] Token 有 `User Profile: Read` 权限
- [ ] Token 未过期
- [ ] Publisher 已创建且状态正常
- [ ] `package.json` 中的 `publisher` 与登录的一致
- [ ] 版本号是新的（未发布过）
- [ ] `vsce verify-pat` 验证成功

## 🚀 快速修复命令

```bash
# 1. 登出
vsce logout QINGYUAI

# 2. 重新登录（使用新 Token）
vsce login QINGYUAI

# 3. 验证权限
vsce verify-pat QINGYUAI

# 4. 发布
vsce publish
```

## 🔗 相关链接

- [Token 设置指南](./Token设置指南.md)
- [权限错误解决方案](./权限错误解决方案.md)
- [打包发布流程](./打包发布流程.md)
- [Azure DevOps Token 管理](https://dev.azure.com/QINGYUAI/_usersSettings/tokens)
- [VSCode Marketplace 管理](https://marketplace.visualstudio.com/manage)

---

**最后更新**: 2025-11-20


